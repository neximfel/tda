...
sec1_main_mid_right_karta_inside5_desc.innerHTML = descpr.map(
    item => `<div class="sec1_main_mid_right_karta_inside5_desc1">
                <div class="sec1_main_mid_right_karta_inside5_desc1_ins">
                    <p>${item.name}</p>
                    <div></div>
                </div>
                <p class="sec1_main_mid_right_karta_inside5_desc1_adds">${item.adds}</p>
            </div>`
).join('');
...
function updateSlider() {
    ...
    // Получаем ширину одной карточки и gap между ними из стилей
    const cardWidth = articles[0].offsetWidth;
    const gap = parseInt(window.getComputedStyle(sec2_main_mid).gap) || 0;

    // Определяем, сколько карточек сейчас видно на экране (по ширине окна)
    let cardsPerPage = 4; // ПК по умолчанию
    if (window.innerWidth <= 768 && window.innerWidth > 480) {
        cardsPerPage = 2; // Планшет
    } else if (window.innerWidth <= 480) {
        cardsPerPage = 1; // Мобильные
    }

    // Вычисляем максимальный индекс страницы
    const maxIndex = Math.ceil(articles.length / cardsPerPage) - 1;

    // Ограничиваем currentIndex, чтобы не листать в пустоту
    if (currentIndex < 0) currentIndex = 0;
    if (currentIndex > maxIndex) currentIndex = maxIndex;

    // Считаем смещение: сдвигаем на целую страницу (ширина карточек + gap)
    const translateX = currentIndex * cardsPerPage * (cardWidth + gap);
    sec2_main_mid.style.transform = `translateX(-${translateX}px)`;

    // Дополнительно: обновление активных точек (dots), если они используются
    if (dots.length > 0) {
...
// Инициализируем слайдер при загрузке (если карточки рендерятся динамически, 
// вызови updateSlider() сразу после того, как карточки добавлены в DOM)
